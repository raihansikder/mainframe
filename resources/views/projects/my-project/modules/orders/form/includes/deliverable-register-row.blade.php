    ...
    <td>
        @if($user->isAdmin())
            ...
            @else
                ...
                @include('form.datetime',['var'=>$varDeliveryDate])
            @endif

        @else
            @if($deliverable->status == 'Delivered')
                <span class="badge bg-green">{{$deliverable->status}}</span><br/>
    ...
    <td>
        @if($deliverable->stockItem)
            @if(!$deliverable->stockItem->warranty_starts_on)
                ...
                @else
                    <a class='btn btn-default bg-blue' target='_blank' href='{{$warrantyRegistrationLink}}'>Register</a>
                @endif
            @else
                <b>Warranty Registered</b> <br/>
                {{formatDate($deliverable->stockItem->warranty_starts_on)}}
                -To- {{formatDate($deliverable->stockItem->warranty_ends_on)}}
            @endif
        @endif
    </td>
</tr>
