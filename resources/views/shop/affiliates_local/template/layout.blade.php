...
<nav class="navbar navbar-inverse navbar-fixed-top pmd-navbar pmd-z-depth">
	...
	<div class="container-fluid">
		...
		<div class="navbar-header">
		  ...
		  <a href="index.html" class="navbar-brand"><img src="{{asset('letsbab/shop/affil/assets/images/logos/') . '/' . $affil_store['storeLogo']}}" class="img-fluid ${3|rounded-top,rounded-right,rounded-bottom,rounded-left,rounded-circle,|}" alt=""></a>
		</div>
	</div>
	...
	<ul class="nav pmd-sidebar-nav">
		...
		<li class="dropdown pmd-dropdown">
			<a aria-expanded="false" data-toggle="dropdown" class="btn-user dropdown-toggle media" data-sidebar="true"
				...
				<span class="media-body">{{$cat['category']}}</span>
				<div class="media-right media-bottom"><i class="dic-more-vert dic"></i></div>
				</a>
			<ul class="dropdown-menu">
					@foreach ($cat['categoryChildren'] as $child_category)
						<li><a class="cat-links" href="#"  data-categorypath="{{$cat['category']}}|{{$child_category['category']}}" data-json="{{ \App\Http\Controllers\Shop\ShopController::slugify($child_category['categoryUrl']) }}.json">{{$child_category['category']}}</a>
						</li>
					@endforeach
			</ul>
		</li>
